...
if (!inputFile || !outputFile) {
  console.log(
    "Usage: bun run scripts/convertOpenThaiGptEvalToJsonl.ts <input-csv> <output-jsonl> [subject]"
  );
  console.log(
    "Example: bun run scripts/convertOpenThaiGptEvalToJsonl.ts datasets/openthaigpt_eval/14_onet_m6_math.csv thai_exam/data/onet/onet_m6_math.jsonl math"
  );
  process.exit(1);
}
...
const jsonlData = records
  .map((record: any, index: number) => {
    ...
    if (!record.year || !record.no || !record.instruction) {
      warnings.push(`Warning: Missing required fields at line ${lineNumber} (CSV row ${
        index + 2
      })
...
Record: ${JSON.stringify(record)}`;
    ...
    if (record.isAnswerable === "FALSE") {
      warnings.push(
        `Warning: Question #${questionNumber} - Skipping non-answerable question (CSV row ${lineNumber})${
    ...
    if (
      record.isMultipleChoice === "TRUE" &&
      record.isSingleChoiceSolution === "TRUE"
    ) {
      // Get options from the input field
      if (!record.input) {
        warnings.push(
          addRecordContext(
            `Warning: Question #${questionNumber} is missing options (CSV row ${lineNumber})`
          )
        );
        skippedCount++;
        return null;
      ...
      if (matches.length < 2) {
        warnings.push(
          addRecordContext(
            `Warning: Question #${questionNumber} has fewer than 2 options extracted (CSV row ${lineNumber})`
          )
        );
        skippedCount++;
        return null;
      ...
      if (record.result) {
        ...
        if (resultMatch) {
          ...
          if (answerNumber >= 1 && answerNumber <= 5) {
            ...
            warnings.push(
              addRecordContext(
                `Warning: Question #${questionNumber} - Answer number out of range: ${answerNumber} (CSV row ${lineNumber})`
              )
            );
          }
        } else {
          warnings.push(
            addRecordContext(
              `Warning: Question #${questionNumber} - Could not parse answer: ${resultText} (CSV row ${lineNumber})`
            )
          );
        }
      } else {
        warnings.push(
          addRecordContext(
            `Warning: Question #${questionNumber} - Missing answer (CSV row ${lineNumber})`
          )
        );
      }
      ...
      if (answer && !optionValues[answer]) {
        warnings.push(
          addRecordContext(
            `Warning: Question #${questionNumber} - Answer '${answer}' doesn't match any option (CSV row ${lineNumber})`
          )
        );
      }
      ...
      if (!record.result) {
        warnings.push(
          addRecordContext(
            `Warning: Question #${questionNumber} - Answerable question is missing an answer (CSV row ${lineNumber})`
          )
        );
        skippedCount++;
        return null;
      ...
      warnings.push(
        `Warning: Question #${questionNumber} - Skipping multiple-choice question without single choice solution (CSV row ${lineNumber})${
          record.note ? ` - Note: ${record.note}` : ""
        }`
      );
      skippedCount++;
      ...
      warnings.push(
        addRecordContext(
          `Warning: Question #${questionNumber} - Skipping question for unknown reason (CSV row ${lineNumber})`
        )
      );
      skippedCount++;
      return null;
...
// Print summary
console.log(
  `Converted ${processedCount} questions from ${inputFileName} to ${outputFileName}`
);
